...
const focusInput = () => {
  ...
  // When the input is focused, ensure the cursor blink is restarted
  terminalStore.startCursorBlink()
}

defineExpose({ focusInput })

// Monitor focus changes to restart cursor blinking when needed
watch(
...
onMounted(() => {
  ...
  watch(() => terminalStore.currentCommand, () => {
    terminalStore.startCursorBlink()
  ...
  <div class="command-prompt">
    ...
    <div class="input-container">
      <input
        ...
        @focus="terminalStore.startCursorBlink()"
        @click="terminalStore.updateCursorPosition(inputRef?.selectionStart || 0)"
        @input="terminalStore.updateCursorPosition(inputRef?.selectionStart || 0)"
        @selectionchange="terminalStore.updateCursorPosition(inputRef?.selectionStart || 0)"
        spellcheck="false"
        autocomplete="off"
        autocorrect="off"
        class="command-input"
      ...
      <span 
        ...
        :style="{ left: (terminalStore.cursorPosition || terminalStore.currentCommand.length) + 'ch' }"
      ></span>
    </div>
  </div>
</template>
